...
Процедура ПриОткрытии(Отказ)
	...
	_АдресаХранилищ = новый Структура("Движения, Подписки, Команды, ОбщиеКоманды, Подсистемы, ОбщиеРеквизиты, РолиИПользователи, ВводНаОсновании, ПоследовательностиДокументов, ТаблицыСХ");
	...
	_АдресаХранилищ.ВводНаОсновании = ПоместитьВоВременноеХранилище(-1, УникальныйИдентификатор);
	_АдресаХранилищ.ПоследовательностиДокументов = ПоместитьВоВременноеХранилище(-1, УникальныйИдентификатор);
	_АдресаХранилищ.ТаблицыСХ = ПоместитьВоВременноеХранилище(-1, УникальныйИдентификатор);
...
Функция вСформироватьСтруктуруУзлаДЗ(ВидУзла="", Имя="", ПолноеИмя="", Синоним="", ЕстьДети=ложь, ПредставлениеОбъекта="", СвойстваОбъекта="")
	Струк = новый Структура("ВидУзла, Имя, ПолноеИмя, Синоним, ПредставлениеОбъекта, СвойстваОбъекта, ЕстьДети, Расширение", ВидУзла, Имя, ПолноеИмя, Синоним, ПредставлениеОбъекта, СвойстваОбъекта, ЕстьДети, ложь);
	Возврат Струк;
КонецФункции
...
Процедура вЗапуститьСеанс1СподПользователем(ТипЗапуска)
	...
	#Иначе
		...
		СтрокаЗапуска = СтрШаблон(СтрокаЗапуска + " /IBConnectionString %1 /N""%2""", СтрокаСоединенияИнформационнойБазы(), мПользовательДляЗапуска1С.Имя);
...
Процедура вЗаполнитьДеревоВстроенныхОбработок()
	...
	Если ДеревоВстроенныеОбработки.ПолучитьЭлементы().Количество() = 0 Тогда
		...
		пСтрук.Вставить("StandardFindByRef"							, "Поиск ссылок на объекты");
