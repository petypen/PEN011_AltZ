...
Асинх Процедура КомандаЗагрузитьИзФайла(Команда)
	...
	Если СтруктураТаблицы.Количество() > 0 Тогда
		Результат = Ждать ВопросАсинх("Перед загрузкой таблица будет очищена. Продолжить?",
	...
	ДиалогВыбораФайла.Заголовок = "Табличный документ с данными для таблицы значений...";
	ДиалогВыбораФайла.Фильтр = "Табличный документ 1С (*.mxl)|*.mxl|Электронная таблица ODF (*.ods)|*.ods";
	Результат = Ждать ДиалогВыбораФайла.ВыбратьАсинх();

	Если Результат = Неопределено Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю("Необходимо выбрать файл с данными для таблицы значений.");
		Возврат;
	КонецЕсли;
	...
	ЗагрузитьТабличныйДокумент(АдресВХранилище, РасширениеФайла);

	ОбщегоНазначенияКлиент.СообщитьПользователю("Загрузка данных в таблицу значений завершена.");
КонецПроцедуры
...
&НаСервере
Процедура ЗагрузитьТабличныйДокумент(АдресВХранилище, РасширениеФайла)

	ТаблицаЗначений = ТабличныйДокументВТаблицуЗначений(АдресВХранилище, РасширениеФайла);
	ЗаполнитьПоляСтруктурыТаблицы(СтруктураТаблицы, ТаблицаЗначений);

КонецПроцедуры

&НаСервере
Функция ТабличныйДокументВТаблицуЗначений(АдресВХранилище, РасширениеФайла)

	ДвоичныеДанные = ПолучитьИзВременногоХранилища(АдресВХранилище); // ДвоичныеДанные - 
	ПотокДляЧтения = ДвоичныеДанные.ОткрытьПотокДляЧтения();

	ТабличныйДокумент = Новый ТабличныйДокумент();
	Если РасширениеФайла = ".ODS" Тогда
		ТипФайлаТД = ТипФайлаТабличногоДокумента.ODS;
	Иначе
		ТипФайлаТД = ТипФайлаТабличногоДокумента.MXL;
	КонецЕсли;
	ТабличныйДокумент.Прочитать(ПотокДляЧтения, СпособЧтенияЗначенийТабличногоДокумента.Текст, ТипФайлаТД);
...
&НаСервере
Процедура ЗаполнитьПоляСтруктурыТаблицы(СтруктураТаблицы, ТаблицаЗначений)
	
	Для Каждого Колонка Из ТаблицаЗначений.Колонки Цикл
		НоваяСтрока = СтруктураТаблицы.Добавить();
		НоваяСтрока.Имя = Колонка.Имя;
		НоваяСтрока.ТипЗначения = Колонка.ТипЗначения;
		НоваяСтрока.Квалификаторы = ПолучитьПредставлениеКвалификаторовТипа(Колонка.ТипЗначения);
	КонецЦикла;
