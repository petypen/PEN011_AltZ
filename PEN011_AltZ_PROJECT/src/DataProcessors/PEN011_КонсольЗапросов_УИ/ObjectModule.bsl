...
Процедура Инициализация(Форма = Неопределено, чСеансИД = Неопределено) Экспорт
	...
	Попытка
		...
		Если Сред(ИспользуемоеИмяФайлаВнешнейОбработки, 2, 2) = ":\" Или Лев(ИспользуемоеИмяФайлаВнешнейОбработки, 1)
			= "/" Тогда
	...
	Для Каждого кз Из стМакроколонки Цикл
		...
		ИначеЕсли кз.Значение.Тип = "ДатаСоздания" Тогда
			...
			Если ЗначениеЗаполнено(Значение) 
				И УИ_ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Значение)) Тогда 
				РезультатЗапросаСтрока[кз.Ключ] = УИ_ОбщегоНазначения.ДатаСозданияСсылки(Значение);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
