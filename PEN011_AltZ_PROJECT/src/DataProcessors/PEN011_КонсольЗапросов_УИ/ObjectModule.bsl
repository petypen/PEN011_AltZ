	...
	Если Форма <> Неопределено Тогда
		...
		Для Каждого Область Из МакетКартинок.Области Цикл
			Если ТипЗнч(Область) = Тип("РисунокТабличногоДокумента") Тогда
				СтруктураКартинки.Вставить(Область.Имя, Область.Картинка);
			КонецЕсли;
		КонецЦикла;

		Картинки = ПоместитьВоВременноеХранилище(СтруктураКартинки, Форма.УникальныйИдентификатор);
...
Функция ПолучитьИмяКартинкиТипа(Значение)
	...
	Для Каждого Колонка Из тзТаблица.Колонки Цикл
		...
		Если Колонка.ТипЗначения.СодержитТип(Тип("Null")) Тогда
			...
			Если маСтрокиСNull.Количество() = 0 Тогда
				...
				тзНоваяТаблица.Колонки.Добавить(Колонка.Имя, Новый ОписаниеТипов(Колонка.ТипЗначения, ,
					маВычитаемыеТипы));
				Продолжить;
			КонецЕсли;
		КонецЕсли;
...
Процедура ОбработатьМакроколонки(РезультатЗапросаСтрока, выбВыборка, стМакроколонки) Экспорт
	...
	Для Каждого кз Из стМакроколонки Цикл
		Если кз.Значение.Тип = "УИД" Тогда
			...
			КонецЕсли;
		ИначеЕсли кз.Значение.Тип = "ДатаСоздания" Тогда
			Значение = РезультатЗапросаСтрока[кз.Значение.КолонкаИсточника];
			Если ЗначениеЗаполнено(Значение) И ЭтоСсылка(ТипЗнч(Значение)) Тогда
				РезультатЗапросаСтрока[кз.Ключ] = ДатаСозданияСсылки(Значение);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ТехнологическийЖурнал_ПолучитьФрагментЖурналаПоИдентификаторуИВремени(Идентификатор, ВремяНачалаЗапроса,
...
Процедура ВыполнитьКод(ЭтотКод, Выборка, Параметры, ПризнакПрогресса)
	Выполнить (ЭтотКод);
КонецПроцедуры
...
Функция СообщитьПрогресс(Обработано, КоличествоВсего, ДатаНачалаВМиллисекундах, ПризнакПрогресса)
	...
	Сообщить(ПризнакПрогресса + ЗначениеВСтроку(Новый Структура("Прогресс, ДлительностьНаМоментПрогресса", Обработано
...
// SQL План В Термины 1С
// 
// Параметры:
//  ПланЗапроса - Строка -  План запроса
//  ДанныеТерминов - ТаблицаЗначений - Данные терминов
//  РегистрТерминов - Число - Регистр терминов, где
// 		0 - не преобразовывать данные терминов
// 		1 - данные терминов преобразовать в нижний регистр (для POSTGRS)
// 
// Возвращаемое значение:
//  Строка - SQLПлан в термины1 С
...
Функция ОбработкаВходитВСоставУниверсальныхИнструментов() Экспорт
	Возврат Метаданные().Имя = "PEN011_КонсольЗапросов_УИ";
КонецФункции
...
Функция ОписаниеТипаВсеСсылки()
	...
	ОписаниеТипаЛюбаяСсылкаXDTO = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип("http://v8.1c.ru/8.1/data/core",
		"TypeDescription"));
	ОписаниеТипаЛюбаяСсылкаXDTO.TypeSet.Добавить(СериализаторXDTO.ЗаписатьXDTO(
		Новый РасширенноеИмяXML("http://v8.1c.ru/8.1/data/enterprise/current-config", "AnyRef")));
