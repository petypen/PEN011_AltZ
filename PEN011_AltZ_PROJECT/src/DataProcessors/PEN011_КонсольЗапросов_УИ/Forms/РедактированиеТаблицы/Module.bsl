...
Процедура ИнициализироватьКонтейнерыСтрокиПоТипам(Знач чСтрока, Знач ТаблицаЗначенийКолонкиКонтейнера)
	РеквизитФормыВЗначение("Объект").ИнициализироватьКонтейнерыСтрокиПоТипам(
		ТаблицаЗначений.НайтиПоИдентификатору(чСтрока), ТаблицаЗначенийКолонкиКонтейнера);
КонецПроцедуры
...
Процедура ОбработатьСериализуемыеЗначения(Знач Структура)
	...
	Если Не ТипЗнч(Значение) = Тип("ТаблицаЗначений") Тогда
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Полученный тип %1. Ожидается ""ТаблицаЗначений""'"), ТипЗнч(Значение));
...
Процедура ЗаполнитьТаблицуЗначений(Значение)
	...
	PEN011_ОбщегоНазначенияВызовСервера.СоздатьРеквизитыТаблицыПоКолонкам(ЭтотОбъект, "ТаблицаЗначений",
		"ТаблицаЗначенийСоответствиеКолонок", "ТаблицаЗначенийКолонкиКонтейнера", Значение.Колонки, Истина);

	Обработка.ТаблицаВРеквизитыФормы(Значение, ТаблицаЗначений, ТаблицаЗначенийКолонкиКонтейнера);
...
Процедура ИмпортироватьЗначениеИзФайла()
	...
	ОписаниеОповещенияПослеВыбора = Новый ОписаниеОповещения("ИмпортироватьЗначениеИзФайлаЗавершение", ЭтотОбъект,
...
Процедура ЭкспортироватьЗначениеВФайл()
	...
	ОписаниеОповещенияПослеВыбора = Новый ОписаниеОповещения("ЭкспортироватьЗначениеВФайлЗавершение", ЭтотОбъект,
		ДополнительныеПараметры);
