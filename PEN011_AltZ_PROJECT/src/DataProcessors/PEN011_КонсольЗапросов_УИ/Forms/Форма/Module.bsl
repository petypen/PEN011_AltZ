...
#Область УИ_ПриСозданииНаСервере
	...
	Если УИ_ВходитВСоставУниверсальныхИнструментов Тогда
		...
		УИ_РедакторКодаСервер.СоздатьЭлементыРедактораКода(ЭтотОбъект, "Алгоритм", Элементы.ТекстАлгоритм, , ,
			Элементы.ГруппаАлгоритмКоманднаяПанельРедактораКода);
		УИ_РедакторКодаСервер.СоздатьЭлементыРедактораКода(ЭтотОбъект, "АлгоритмПередВыполнением",
			Элементы.ТекстАлгоритмПередВыполнением, , , Элементы.ГруппаАлгоритмПередВыполнениемКоманднаяПанель);
		УИ_РедакторКодаСервер.СоздатьЭлементыРедактораКода(ЭтотОбъект, "Запрос", Элементы.ТекстЗапроса, , "bsl_query",
...
Процедура Команда_СинтаксическийКонтрольЗапроса(Команда)
	...
	Если ТипЗнч(Результат) <> Тип("Строка") Тогда
		...
		Если ЗначениеЗаполнено(Результат.Строка) Тогда
			УстановитьГраницыВыделенияЗапроса(Результат.Строка, Результат.Колонка, Результат.Строка, Результат.Колонка);
		КонецЕсли;
	КонецЕсли;
...
Процедура Команда_ПараметрыЗаполнитьИзЗапроса(Команда)
	...
	Если ЗначениеЗаполнено(стОшибка) Тогда
		...
		Если ЗначениеЗаполнено(стОшибка.Строка) Тогда
			УстановитьГраницыВыделенияЗапроса(стОшибка.Строка, стОшибка.Колонка, стОшибка.Строка, стОшибка.Колонка);
		КонецЕсли;
...
Процедура Команда_ВыполнитьОбработку(Команда)
	...
	УИ_РедакторКодаКлиент.НачатьСборкуОбработокДляИсполненияКода(ЭтотОбъект,
		Новый ОписаниеОповещения("Команда_ВыполнитьОбработкуЗавершениеПодключенияОбработокИсполнения", ЭтотОбъект),
...
Функция ИзвлечьРезультатВДанныеФормы(РезультатВПакете)
	...
	Если Число(РезультатВПакете) <= 0 Тогда
		PEN011_ОбщегоНазначенияВызовСервера.СоздатьРеквизитыТаблицыПоКолонкам(ЭтотОбъект, ИмяРеквизитаРезультата,
...
Процедура ВыполнитьАлгоритмПередВыполнениемЗапроса(Запрос, ТекстАлгоритма)
	...
	Результат = УИ_РедакторКодаКлиентСервер.ВыполнитьАлгоритм(ТекстАлгоритма, Контекст, Ложь, ЭтотОбъект,
...
Функция ВыполнитьЗапросНаСервере(ТекстЗапросаДляВыполнения, ТекстАлгоритмаПередВыполнением)
	...
	Исключение
		СтрокаОшибки = ОписаниеОшибки();
		РазобратьОшибкуЗапросаСУточнением(СтрокаОшибки, ВыполняемыйЗапрос, ТекстЗапросаДляВыполнения, НомерСтроки,
			НомерКолонки);
	...
	Исключение
		СтрокаОшибки = ОписаниеОшибки();
		РазобратьОшибкуЗапросаСУточнением(СтрокаОшибки, ВыполняемыйЗапрос, ВыполняемыйЗапрос.Текст, НомерСтроки,
			НомерКолонки);
		Возврат Новый Структура("ОписаниеОшибки, Строка, Колонка, ВремяНачала, ВремяОкончания", СтрокаОшибки,
			НомерСтроки, НомерКолонки);
	КонецПопытки;
...
Процедура ПакетЗапросов_ЗагрузитьЗавершение(ДополнительныеПараметры) Экспорт
	...
	Если ТекущаяСтрока <> Неопределено И Элементы.ПакетЗапросов.ТекущаяСтрока <> ТекущаяСтрока Тогда
...
Функция Запрос_ПолучитьДанныеЗапроса(идЗапрос)
	...
	Возврат Новый Структура("Имя, Запрос, ТекстКод, МетодИсполненияКода, Параметры, ВременныеТаблицы, 
		...
		СтрокаЗапроса.КодКурсорКонецКолонки + 1, СтрокаЗапроса.ТекстЗапросаОригинальный,
		СтрокаЗапроса.ТекстКодОригинальный, СтрокаЗапроса.АлгоритмПередВыполнением,
		СтрокаЗапроса.АлгоритмПередВыполнениемОригинальный);
...
Процедура ИзменитьИмяПараметраВТекстеЗапроса(РезультатВопроса, ДополнительныеПараметры) Экспорт
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		УстановитьТекстЗапроса(ЗаменитьПараметр(ТекущийТекстЗапроса(), "&"
...
Процедура ВыполнитьЗапрос(фИспользоватьВыделение)
	...
	УИ_РедакторКодаКлиент.НачатьСборкуОбработокДляИсполненияКода(ЭтотОбъект,
		Новый ОписаниеОповещения("ВыполнитьЗапросЗавершениеСохраненияОбработокИсполненияАлгоритмов", ЭтотОбъект,
		ПараметрыОповещения), Новый Структура("АлгоритмПередВыполнением", ИменаПеременных));
...
Процедура ВыполнитьЗапросЗавершениеСохраненияОбработокИсполненияАлгоритмов(Результат, ДополнительныеПараметры) Экспорт
	ГраницыВыделенияЗапроса = ГраницыВыделенияЗапроса();
	фВесьТекст = Не ДополнительныеПараметры.фИспользоватьВыделение Или (ГраницыВыделенияЗапроса.НачалоСтроки
		= ГраницыВыделенияЗапроса.КонецСтроки И ГраницыВыделенияЗапроса.НачалоКолонки
		= ГраницыВыделенияЗапроса.КонецКолонки);
	...
	Если ЗначениеЗаполнено(стРезультат.ОписаниеОшибки) Тогда
		...
		Если ЗначениеЗаполнено(стРезультат.Строка) Тогда
			Если фВесьТекст Тогда
				УстановитьГраницыВыделенияЗапроса(стРезультат.Строка, стРезультат.Колонка, стРезультат.Строка,
					стРезультат.Колонка);
			Иначе
			КонецЕсли;
		КонецЕсли;
...
Функция ВыполнитьАлгоритм(Алгоритм)
	...
	РезультатВыполнения = УИ_РедакторКодаКлиентСервер.ВыполнитьАлгоритм(Алгоритм, Контекст, Ложь, ЭтотОбъект,
		"Алгоритм");
...
Функция ВыполнитьАлгоритмПострочно(АдресРезультатаЗапроса, РезультатВПакете, ТекстАлгоритма)
	...
	Пока выбВыборка.Следующий() Цикл
		РезультатВыполнения = УИ_РедакторКодаКлиентСервер.ВыполнитьАлгоритм(ТекстАлгоритма, Контекст, Ложь, ЭтотОбъект,
			"Алгоритм");
...
Процедура УстановитьТекстАлгоритма(НовыйТекст, УстанавливатьОригинальныйТекст = Ложь, НовыйОригинальныйТекст = "")
	Если УИ_ВходитВСоставУниверсальныхИнструментов Тогда
		УИ_РедакторКодаКлиент.УстановитьТекстРедактора(ЭтотОбъект, "Алгоритм", НовыйТекст,
...
Процедура УИ_ДобавитьКонтекстАлгоритмаПередВыполнением()
	...
	УИ_РедакторКодаКлиент.ДобавитьКонтекстРедактораКода(ЭтотОбъект, "АлгоритмПередВыполнением",
